...
function submitQuiz() {
    ...
    const timeText = `⏱ Thời gian làm bài: ${minutes} phút ${seconds} giây`;
...
function printResult() {
    ...
    quizData.forEach((row, index) => {
        const userAns = userAnswers[index] || "Chưa chọn";
        const correct = row[5];
        const nghiepVu = row[8] || "Không rõ";

        content += `
            <p><b>Câu ${index + 1}:</b> ${row[0]}</p>
            <p style="color:blue;">📌 Nghiệp vụ: ${nghiepVu}</p>
            ...
            <p>✔ Bạn chọn: <b>${userAns}</b></p>
            <p>👉 Đáp án đúng: <b>${correct}</b></p>
